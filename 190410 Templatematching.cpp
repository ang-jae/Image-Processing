...
void main_0409()//190409 DownSampling
{
	...
	DownSize2(image, height, width, image_out);
...
POS2D TemplateMatching(int**image, int height, int width, int**block, int height_t, int width_t)
{
	int Err;
	int Err_min = 99999;
	POS2D pos;
	...
	printf("\nx = %f\ny = %f\nErr_min = %d", pos.x, pos.y, Err_min);
	return pos;
}

void main()
{
	int height, width, height_t, width_t;
	int** image = ReadImage((char*)"Koala.bmp", &height, &width);
	int** block = ReadImage((char*)"template.bmp", &height_t, &width_t);
	POS2D pos;
	pos = TemplateMatching(image, height, width, block, height_t, width_t);
	waitKey(0);
}