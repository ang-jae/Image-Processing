...
void ChangeBlock(int**block, int**block_changed, int height_b, int width_b, int num)
//num 1:가만히 2:x축대칭 3:y축대칭 4:원점대칭 5:대각선대칭(/) 6:대각선대칭(반대) 7:90도 회전 8:180도 회전
{	
//	printf("\nnum\n1:가만히\n2:x축대칭\n3:y축대칭\n4:원점대칭\n5:대각선대칭(/)\n6:대각선대칭(반대)\n7:90도 회전\n8:180도 회전\nnum = ");
//	scanf_s("%d", &num);
	switch (num) {
...
void main()
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		ChangeBlock(block, block_changed[i], height_t, width_t, i+1);
	...
	for (int i = 0; i < 8; i++)	printf("\n%d.err = %d, pos = (%f, %f)", i,err[i], pos[i].x, pos[i].y);
	ImageShow((char*)"image", image, height, width);
