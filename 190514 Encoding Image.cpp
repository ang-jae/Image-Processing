...
POS2D BlockCompareWithDblocks(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int y, int x, int N, int* avg)
{
	POS2D pos;
	int Err_min = INT_MAX;
	//block 1
	Cutting(image, height, width, y, x, N, block);
	int avg = GetBlockAvg(image, N, N, y, x, N);
	...
	for (int i = 0; i < height - (2 * N); i++)
	{
		...
		{
			Cutting(image, height, width, i+y, j+x, 2 * N, Dblock);
...
POS2D EncodingImage(int**image, int height, int width, int**block, int**block_mean, 
	int** Dblock, int**Dblock2, int**Dblock2_mean, int N, int*avg)
{
	POS2D pos;
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			pos = BlockCompareWithDblocks(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, i, j, N, avg);
		}
	}
	return pos;
}
...
void main()
{
	...
	POS2D pos;
	pos = EncodingImage(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, N);

	printf("%f, %f, avg = %d", pos.x, pos.y, avg);
}