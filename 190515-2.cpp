...
Block SetBlock(int x, int y, int avg, int Davg, int gt)
{
	Block tmp;
	tmp.x = x; tmp.y = y; tmp.avg = avg; tmp.Davg = Davg; tmp.gt = gt;
	return (tmp);
}
	...
	int** Dblock, int**Dblock2, int**Dblock2_mean, int y, int x, int N)//x, y : block의 위치
{
	...
	for (int i = 0; i < height - (2 * N); i++)
	{
		for (int j = 0; j < width - (2 * N); j++)
		{
			...
			Block.Davg = GetBlockAvg(Dblock2, N, N, i + y, j + x, N);
	...
	int** Dblock, int**Dblock2, int**Dblock2_mean, int N)
{
	...
	for (int i = 0; i < height - N; i += N)
	{
		for (int j = 0; j < width - N; j += N)
		{
			...
			{
				if (Blocks[k].x >= width || Blocks[k].y >= height) continue;
				else Blocks[k] = BlockCompareWithDblocks(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, i, j, N);
			}
		}
	}
	return Blocks;
