	...
	int** Dblock, int**Dblock2, int**Dblock2_mean, int N)
{
	...
	for (int i = 0; i < height - N; i += N)
	{
		for (int j = 0; j < width - N; j += N)
		{
			...
			printf("\nBlocks[%d]: (x, y) = (%d, %d), avg = %d", k, Blocks[k].x, Blocks[k].y, Blocks[k].avg);
			k++;
		}
	}
	return Blocks;
	...
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_gt, int N)
{
	...
	for (int i = 0; i < height - N; i += N)
	{
		for (int j = 0; j < width - N; j += N)
		{
			Blocks[k] = FindBestGTofDblock(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, i, j, N);
			Blocks[k] = BlockCompareWithGeometricTransformedDblocks(image, height, width, block, block_mean, Dblock, Dblock2,
				Dblock2_mean, Dblock2_mean_gt, i, j, N, Blocks[k].gt);
			printf("\nBlocks[%d]: (x, y) = (%d, %d), avg = %d", k, Blocks[k].x, Blocks[k].y, Blocks[k].avg);
			k++;
		}
	}
...
void main()
{
	...
	*/
	...
	Blocks = EncodingImageWithGeometricTransform(image, height, width, block, block_mean,
		Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, N);
