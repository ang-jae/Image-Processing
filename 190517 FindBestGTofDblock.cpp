...
Block BlockCompareWithGeometricTransformedDblocks(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_gt, int y, int x, int N, int gt)
{
	int Err_min = INT_MAX;
	int length = height * width / (N * N);
	Block Block;
	...
	for (int i = 0; i < height - N; i++)
	{
		for (int j = 0; j < width - N; j++)
		{
			...
			DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
			GeometricTransform(Dblock2_mean, Dblock2_mean_gt, N, N, gt);
			Block.error = FindErr(block_mean, Dblock2_mean, 0, 0, N, N, N, N);
			if (Block.error < Err_min)
			{
				Err_min = Block.error;
...
int FindBestGTofDblock(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_gt, int y, int x, int N)
{
	...
	for (int i = 0 ; i < 8 ; i++)
	{
		Block = BlockCompareWithGeometricTransformedDblocks(image, height, width, block, block_mean,
			Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, y, x, N, i);
		if (Block.error < Err_min)
		{
			Err_min = Block.error;
			gt = i;
...
void main()
{
	...
	for(int i = 0; i < height ; i+=N)
		for (int j = 0; j < width; j += N)
		{
			gt = FindBestGTofDblock(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, 0, 0, N);
			printf("\nTheBestGtofDblock is %d", gt);
		}
	...
	ImageShow((char*)"Dblock", Dblock2, N, N);
