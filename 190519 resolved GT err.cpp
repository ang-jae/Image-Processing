...
void Cutting(int**image, int height, int width, int y, int x, int N, int**block)
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
...
Block SetBlock(int x, int y, int avg, int gt, int error)
{
	Block tmp;
	tmp.x = x; tmp.y = y; tmp.avg = avg; tmp.gt = gt; tmp.error = error;
	return (tmp);
}
...
void main()
{
	...
	Block**Blocks_temp = BlockAlloc2(height / N, width / N);
	...
	*/
	...
	Blocks = EncodingImageWithGeometricTransform(image, height, width, block, block_mean, Dblock, Dblock2, 
		Dblock2_mean, Dblock2_mean_gt, Blocks_temp, N);

	//DecodingImage(input_image, Dblock, Dblock2, Dblock2_mean, width2, height2, Blocks, width, height, decoded_image, N);

	DecodingImageWithGeometricTransform(image, Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, width2, height2,
		Blocks, width, height, decoded_image, N);
