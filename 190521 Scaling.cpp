...
void Scaling(int**image, int height, int width, float alpha, int**image_scaled)
{
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			image_scaled[i][j] = alpha * image[i][j];
		}
	}
}
	...
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_scaled, int y, int x, int N)
{
	...
	{
		Block = BlockCompareWithScaledDblocks(image, height, width, block, block_mean,
			Dblock, Dblock2, Dblock2_mean, Dblock2_mean_scaled, y, x, N, alpha);
		//printf("\ni = %d, error = %d", geo_trans, Block.error);
		if (Block.error < Err_min)
		{
			Err_min = Block.error;
	...
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
		{
			...
			for (int k = 0; k < N; k++)
			{
				...
				{
					if (i + k >= height_out || j + l >= width_out) continue;
					else decoded_image[i + k][j + l] = Dblock2_mean[k][l] + Blocks[i / N][j / N].avg;
				}
			}
		}
	}
...
void main()
{
	...
	N = 8;
	int** image = ReadImage((char*)"LENA256.bmp", &height, &width);
	int** input_image = ReadImage((char*)"Koala.bmp", &height2, &width2);
