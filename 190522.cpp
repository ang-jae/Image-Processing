	...
	int** Dblock, int**Dblock2, int**Dblock2_mean, int y, int x, int N)//x, y : block의 위치
{
	...
	Block.avg = (int)GetBlockAvg(block, N, N, 0, 0, N);
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			Block.error = FindErr(block_mean, Dblock2_mean, 0, 0, N, N, N, N);
			if (Block.error < Err_min)
			{
				Err_min = Block.error;
				Block.x = j;
				Block.y = i;
...
Block BlockCompareWithGeometricTransformedDblocks(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_gt, int y, int x, int N, int geo_trans)
{
	int Err_min = INT_MAX;
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			Block.error = FindErr(block_mean, Dblock2_mean_gt, 0, 0, N, N, N, N);
			if (Block.error < Err_min)
			{
				Err_min = Block.error;
				Block.x = j;
...
Block FindBestGTofDblock(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_gt, int y, int x, int N)
{
	Block Block;
	int Err_min = INT_MAX;
	...
	{
		Block = BlockCompareWithGeometricTransformedDblocks(image, height, width, block, block_mean,
			Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, y, x, N, geo_trans);
	...
	Block.gt = geo_trans_out;
	return Block;
}

void Scaling(int**image, int height, int width, float alpha, int**image_scaled)
{
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			image_scaled[i][j] = (float)(alpha * (float)image[i][j]);
		}
	}
}

Block BlockCompareWithScaledDblocks(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_scaled, int y, int x, int N, float alpha)
{
	int Err_min = INT_MAX;
	int length = height * width / (N * N);
	Block Block;
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
			Scaling(Dblock2_mean, N, N, alpha, Dblock2_mean_scaled);
			Block.error = FindErr(block_mean, Dblock2_mean_scaled, 0, 0, N, N, N, N);
			if (Block.error < Err_min)
			{
				Err_min = Block.error;
				Block.x = j;
...
Block FindBestAlphaofDblock(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_scaled, int y, int x, int N)
{
	Block Block;
	int Err_min = INT_MAX;
	float alpha, alpha_out;
	for (alpha = 0.3; alpha <= 1.0; alpha += 0.1)
	{
		Block = BlockCompareWithScaledDblocks(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, Dblock2_mean_scaled, y, x, N, alpha);
		if (Block.error < Err_min)
		{
			Err_min = Block.error;
			alpha_out = alpha;
			printf("\nif문 %f 동작, alpha = %f", alpha, Block.alpha);
		}
	}
	Block.alpha = alpha_out;
	return Block;
}

Block**EncodingImage(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, Block**Blocks, int N)
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			Blocks[i / N][j / N] = BlockCompareWithDblocks(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, i, j, N);
...
Block**EncodingImageWithGeometricTransform(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_gt, Block**Blocks, int N)
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			Blocks[i / N][j / N] = FindBestGTofDblock(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, i, j, N);
...
Block**EncodingImageWithScaling(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_scaled, Block**Blocks, int N)
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			Blocks[i / N][j / N] = FindBestAlphaofDblock(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, Dblock2_mean_scaled, i, j,N);
...
void DecodingImage(int** input_image, int**Dblock, int**Dblock2, int**Dblock2_mean,
...
void main()
{
	int height, width, height2, width2, N, length;
	Block Block1;
	N = 8;
	int** image = ReadImage((char*)"LENA256.bmp", &height, &width);
	...
	*/
	Blocks_Scaled = EncodingImageWithScaling(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, Dblock2_mean_scaled, Blocks, N);
	Blocks_GT = EncodingImageWithGeometricTransform(image, height, width, block, block_mean, Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, Blocks, N);
