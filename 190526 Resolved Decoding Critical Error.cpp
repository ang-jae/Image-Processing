...
void DecodingImage(int**image_input, int height_in, int width_in, int height_out, int width_out)
{
...
void DecodingImageWithGeometricTransform(int width_in, int height_in, int width_out, int height_out)
{
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
...
void DecodingImageWithScaling(int height_in, int width_in, int height_out, int width_out)
{
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
...
void main()
{
	...
	*/
	...
	DecodingImage(input_image, height, width, height, width);

	ImageShow((char*)"DecodedImageNormal", decoded_image, height, width);

	for(int i = 0; i < 1000; i++) DecodingImage(decoded_image, height, width, height, width);
	
	//Blocks_Scaled = EncodingImageWithScaling();
	//DecodingImageWithScaling(height, width, height, width);

	//Blocks_GT = EncodingImageWithGeometricTransform();
	//DecodingImageWithGeometricTransform(height, width, height, width);
