...
int** image = ReadImage((char*)"LENA256.bmp", &height, &width);
int** input_image = ReadImage((char*)"white.jpg", &height, &width);
...
void DecodingImageWithGeometricTransform(int width_in, int height_in, int width_out, int height_out, int**decoded_image_gt)
{
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
		{
			ReadBlock(input_image, height_in, width_in, Blocks_GT[i / N][j / N].y, Blocks_GT[i / N][j / N].x, 2 * N, 2 * N, Dblock);
			DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
...
void DecodingImageWithScaling(int height_in, int width_in, int height_out, int width_out, int**decoded_image_scaling)
{
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
		{
			ReadBlock(input_image, height_in, width_in, Blocks_Scaled[i / N][j / N].y, Blocks_Scaled[i / N][j / N].x, 2 * N, 2 * N, Dblock);
			DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
...
void main()
{
	...
	*/
	...
	for(int i = 0; i < 1000; i++) DecodingImage(decoded_image, height, width, height, width, decoded_image);
	ImageShow((char*)"100Decoding", decoded_image, height, width);
	*/

	Blocks_Scaled = EncodingImageWithScaling();
	DecodingImageWithScaling(height, width, height, width, decoded_image_scaling);

	//Blocks_GT = EncodingImageWithGeometricTransform();
	//DecodingImageWithGeometricTransform(height, width, height, width, decoded_image_gt);
	...
	*/
	...
	ImageShow((char*)"DecodedX100ImageNormal", decoded_image, height, width);
	ImageShow((char*)"DecodedImageScaling", decoded_image_scaling, height, width);
	ImageShow((char*)"DecodedImageGT", decoded_image_gt, height, width);
