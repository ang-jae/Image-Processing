...
int**** Dblocks = IntAlloc4(height/(2 * N), width/(2 * N), 2 * N, 2 * N);
int**** Dblocks2 = IntAlloc4(height / (2 * N), width / (2 * N), 2 * N, 2 * N);
int**** Dblocks2_mean = IntAlloc4(height / (2 * N), width / (2 * N), 2 * N, 2 * N);
int**** Dblocks2_mean_gt = IntAlloc4(height / (2 * N), width / (2 * N), 2 * N, 2 * N);
int**** Dblocks2_mean_scaled = IntAlloc4(height / (2 * N), width / (2 * N), 2 * N, 2 * N);
int**** Dblocks2_mean_total = IntAlloc4(height / (2 * N), width / (2 * N), 2 * N, 2 * N);
...
Block BlockCompareWithDblocks(int**image_input, int y, int x)//x, y : block의 위치
{
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			Block_normal.error = FindErr(block_mean, Dblocks2_mean[i/(2*N)][j/(2*N)], 0, 0, N, N, N, N);
...
Block BlockCompareWithGeometricTransformedDblocks(int y, int x, int geo_trans)
{
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			GeometricTransform(Dblocks2_mean[i/(2*N)][j/(2*N)], Dblocks2_mean_gt[i / (2 * N)][j / (2 * N)], N, N, geo_trans);
...
Block BlockCompareWithScaledDblocks(int y, int x, float alpha)
{
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			Scaling(Dblocks2_mean[i / (2 * N)][j / (2 * N)], alpha, N, N, Dblocks2_mean_scaled[i / (2 * N)][j / (2 * N)]);
...
Block BlockCompareWithTotalDblocks(int y, int x, int geo_trans, float alpha)
{
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		...
		{
			Scaling(Dblocks2_mean[i/(2*N)][j/(2*N)], alpha, N, N, Dblocks2_mean_scaled[i/(2*N)][j/(2*N)]);
			GeometricTransform(Dblocks2_mean_scaled[i / (2 * N)][j / (2 * N)], Dblocks2_mean_total[i / (2 * N)][j / (2 * N)], N, N, geo_trans);
			Block_Total.error = FindErr(block_mean, Dblocks2_mean_total[i / (2 * N)][j / (2 * N)], 0, 0, N, N, N, N);
...
void EncodingImage(int**image_input)
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			Blocks[i / N][j / N] = BlockCompareWithDblocks(image_input, i, j);
...
void main()
{
	...
	for (int i = 0; i < 100; i++)
	{
		...
		//	printf("\n PSNR_scaling = %f", PSNR(image, decoded_image_scaling, height, width));
		//	printf("\n PSNR_gt = %f", PSNR(image, decoded_image_gt, height, width));
