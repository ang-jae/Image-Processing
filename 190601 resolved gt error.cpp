...
int** image = ReadImage((char*)"LENA256.bmp", &height, &width);
int** input_image = ReadImage((char*)"Koala256.bmp", &height, &width);
...
int**** Dblocks2 = IntAlloc4(height, width, 2 * N, 2 * N);
int**** Dblocks2_mean = IntAlloc4(height, width, 2 * N, 2 * N);
int**** Dblocks2_mean_gt = IntAlloc4(height, width, 2 * N, 2 * N);
int**** Dblocks2_mean_scaled = IntAlloc4(height, width, 2 * N, 2 * N);
int**** Dblocks2_mean_total = IntAlloc4(height, width, 2 * N, 2 * N);
...
Block FindBestGTandAlphaofDblock(int y, int x)
{
	int Err_min = INT_MAX;
	int geo_trans_out;
	float alpha_out;
	for (float alpha = 0.3; alpha < 1.1; alpha += 0.1)
	{
		...
		{
			BlockCompareWithTotalDblocks(y, x, geo_trans, alpha);
			if (Block_Total.error < Err_min)
			{
				Err_min = Block_Total.error;
				alpha_out = alpha;
...
void EncodingImage()
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			Blocks[i / N][j / N] = BlockCompareWithDblocks(image, i, j);
...
void EncodingImageWithGeometricTransform()
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			Blocks_GT[i / N][j / N] = FindBestGTofDblock(i, j);
			printf("\nBlocks_gt[%d][%d]: (x, y) = (%d, %d), avg = %d, gt = %d", i / N, j / N, Blocks_GT[i / N][j / N].x, Blocks_GT[i / N][j / N].y, Blocks_GT[i / N][j / N].avg, Blocks_GT[i / N][j / N].gt);
		}
	}
}
...
void EncodingImageWithScaling()
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			Blocks_Scaled[i / N][j / N] = FindBestAlphaofDblock(i, j);
			printf("\nBlocks_Scaled[%d][%d]: (x, y) = (%d, %d), avg = %d, alpha = %f", i / N, j / N, Blocks_Scaled[i / N][j / N].x, Blocks_Scaled[i / N][j / N].y, Blocks_Scaled[i / N][j / N].avg, Blocks_Scaled[i / N][j / N].alpha);
		}
	}
}
...
void EncodingImageTotal()
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			Blocks_Total[i / N][j / N] = FindBestGTandAlphaofDblock(i, j);
			printf("\nBlocks_Total[%d][%d]: (x, y) = (%d, %d), avg = %d, alpha = %f, gt = %d", i / N, j / N, Blocks_Total[i / N][j / N].x, Blocks_Total[i / N][j / N].y, Blocks_Total[i / N][j / N].avg, Blocks_Total[i / N][j / N].alpha, Blocks_Total[i / N][j / N].gt);
		}
	}
}
...
void main()
{
	...
	for (int i = 0; i < 100; i++)
	{
		...
		DecodingImageWithScaling(tmp, height, width, height, width, decoded_image_scaling);
		DecodingImageWithGeometricTransform(tmp, height, width, height, width, decoded_image_gt);
		DecodingImageTotal(tmp2, height, width, height, width, decoded_image_total);

		CopyImage(decoded_image, height, width, tmp);
		CopyImage(decoded_image_total, height, width, tmp2);
		CopyImage(decoded_image_scaling, height, width, tmp3);
		CopyImage(decoded_image_gt, height, width, tmp4);
