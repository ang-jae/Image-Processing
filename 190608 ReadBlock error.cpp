...
//num 0:가만히 1:x축대칭 2:y축대칭 3:원점대칭 4:대각선대칭(/) 5:대각선대칭(반대) 6:90도 회전 7:180도 회전
{
	...
	case 1:
		for (int i = 0; i < height_b; i++) {
			for (int j = 0; j < width_b; j++) {
				if (j > height_b || i > width_b || height_b - 1 - i < 0) continue;
				else block_changed[i][j] = block[height_b - 1 - i][j];
			}
		}
		break;
	case 2:
		for (int i = 0; i < height_b; i++) {
			for (int j = 0; j < width_b; j++) {
				if (j > height_b || i > width_b || width_b - 1 - j < 0) continue;
				else block_changed[i][j] = block[i][width_b - 1 - j];
			}
		}
		break;
	case 3:
	...
	case 5:
		for (int i = 0; i < height_b; i++) {
			for (int j = 0; j < width_b; j++) {
				if (j > height_b || j < 0 || height_b - 1 - i > width_b || height_b - 1 - i < 0) continue;
				else block_changed[i][j] = block[j][height_b - 1 - i];
			}
		}
		break;
	case 6:
...
void ReadBlock(int**image_input, int height_image, int width_image, int y, int x, int dy, int dx, int**block)
{
	for (int i = 0; i < dy; i++)
	{
		for (int j = 0; j < dx; j++)
		{
			if (y + i >= height_image || x + j >= width_image) block[i][j] = 0;
			else block[i][j] = image_input[y + i][x + j];
		}
	}
}
...
void WriteBlock(int**image, int height, int width, int y, int x, int dy, int dx, int**block)
{
	for (int i = 0; i < dy; i++)
	{
		for (int j = 0; j < dx; j++)
		{
			if (y + i >= height || x + j >= width) block[i][j] = 0;
...
int**** Dblocks = IntAlloc4(height - (2 * N), width - (2 * N), N, N);
int**** Dblocks2 = IntAlloc4(height - (2 * N), width - (2 * N), N, N);
int**** Dblocks2_mean = IntAlloc4(height - (2 * N), width - (2 * N), N, N);
int**** Dblocks2_mean_gt = IntAlloc4(height - (2 * N), width - (2 * N), N, N);
int**** Dblocks2_mean_scaled = IntAlloc4(height - (2 * N), width - (2 * N), N, N);
int**** Dblocks2_mean_total = IntAlloc4(height - (2 * N), width - (2 * N), N, N);
void FindDblocks2_mean()
{
	for (int i = 0; i < height - 2 * N; i++)
	{
		...
		{
			ReadBlock(image, height, width, i, j, 2 * N, 2 * N, Dblocks[i][j]);
			DownSize2(Dblocks[i][j], 2 * N, 2 * N, Dblocks2[i][j]);
			RemoveMean(Dblocks2[i][j], N, Dblocks2_mean[i][j]);
		}
	}
}
...
{
	int Err_min = INT_MAX, x_temp, y_temp;
	//block
	ReadBlock(image, height, width, y, x, N, N, block);
	Block_normal.avg = RemoveMean(block, N, block_mean);
	//Dblocks
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			Block_normal.error = FindErr(block_mean, Dblocks2_mean[i][j], 0, 0, N, N, N, N);
	...
	Block_normal.x = x_temp;
	Block_normal.y = y_temp;
	return Block_normal;
}
...
Block BlockCompareWithGeometricTransformedDblocks(int y, int x, int geo_trans)
{
	...
	//block
	ReadBlock(image, height, width, y, x, N, N, block);
	Block_gt.avg = RemoveMean(block, N, block_mean); 
	int x_temp, y_temp;
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			//		DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			//		RemoveMean(Dblock2, N, Dblock2_mean);
			GeometricTransform(Dblocks2_mean[i][j], Dblocks2_mean_gt[i][j], N, N, geo_trans);
			Block_gt.error = FindErr(block_mean, Dblocks2_mean_gt[i][j], 0, 0, N, N, N, N);
...
Block FindBestGTofDblock(int y, int x)
{
	...
	for (geo_trans = 0; geo_trans < 8; geo_trans++)
	{
		...
		if (Block_gt.error < Err_min)
		{
			...
			//printf("\nif문 %d 동작, gt = %d", geo_trans, Block.gt);
		}
	}
...
Block BlockCompareWithScaledDblocks(int y, int x, float alpha)
{
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			//	DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			//	RemoveMean(Dblock2, N, Dblock2_mean);
			Scaling(Dblocks2_mean[i][j], alpha, N, N, Dblocks2_mean_scaled[i][j]);
			Block_Scaled.error = FindErr(block_mean, Dblocks2_mean_scaled[i][j], 0, 0, N, N, N, N);
...
Block FindBestAlphaofDblock(int y, int x)
{
	...
	{
		Block_Scaled = BlockCompareWithScaledDblocks(y, x, alpha);
...
Block BlockCompareWithTotalDblocks(int y, int x, int geo_trans, float alpha)
{
	int Err_min = INT_MAX;
	int x_temp = 0, y_temp = 0; // if문 안에서 구조체로 전달이 잘 되지 않아서 추가
								//block
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			//	RemoveMean(Dblock2, N, Dblock2_mean);
			Scaling(Dblocks2_mean[i][j], alpha, N, N, Dblocks2_mean_scaled[i][j]);
			GeometricTransform(Dblocks2_mean_scaled[i][j], Dblocks2_mean_total[i][j], N, N, geo_trans);
			Block_Total.error = FindErr(block_mean, Dblocks2_mean_total[i][j], 0, 0, N, N, N, N);
			if (Block_Total.error < Err_min)
			{
				Err_min = Block_Total.error;
				x_temp = j;
				y_temp = i;
				//printf("\nif문 발동, (x, y) = (%d, %d)", x_temp, y_temp);
			}
		}
	}
	...
	Block_Total.alpha = alpha;
	Block_Total.gt = geo_trans;
	return Block_Total;
}
void CuttingImage(int**image_input, int N, int**im1, int**im2, int**im3, int**im4) //맨 왼쪽 위부터 오른쪽 아래 방향으로
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			...
			else if (i > N / 2 && j < N / 2) im2[i][j] = image_input[i][j + N / 2];
			else if (i > N / 2 && j < N / 2) im3[i][j] = image_input[i + N / 2][j];
			else if (i > N / 2 && j > N / 2) im4[i][j] = image_input[i + N / 2][j + N / 2];
		}
	}
}
...
Block FindBestTotalDblock(int y, int x)
{
	int Err_min = INT_MAX;
	int geo_trans_out;
	float alpha_out;
	for (float alpha = 0.3; alpha < 1.1; alpha += 0.1)
	{
		...
		{
			Block_Total = BlockCompareWithTotalDblocks(y, x, geo_trans, alpha);
			if (Block_Total.error < Err_min)
			{
				Err_min = Block_Total.error;
				alpha_out = alpha;
...
void EncodingImageWithGeometricTransform()
{
	for (int i = 0; i < height / N; i++)
	{
		for (int j = 0; j < width / N; j++)
		{
			Blocks_GT[i][j] = FindBestGTofDblock(i*N, j*N);
...
void EncodingImageWithScaling()
{
	for (int i = 0; i < height / N; i++)
	{
		for (int j = 0; j < width / N; j++)
		{
			Blocks_Scaled[i][j] = FindBestAlphaofDblock(i*N, j*N);
...
void EncodingImageTotal()
{
	for (int i = 0; i < height / N; i++)
	{
		for (int j = 0; j < width / N; j++)
		{
			Blocks_Total[i][j] = FindBestTotalDblock(i*N, j*N);
...
void main()
{
	FindDblocks2_mean();
	EncodingImage();
	EncodingImageWithScaling();
	EncodingImageWithGeometricTransform();
	EncodingImageTotal();

	int** tmp = (int**)IntAlloc2(height, width);
	int** tmp2 = (int**)IntAlloc2(height, width);
	int** tmp3 = (int**)IntAlloc2(height, width);
	int** tmp4 = (int**)IntAlloc2(height, width);

	for (int i = 0; i < 100; i++)
	{
		DecodingImage(tmp, height, width, height, width, decoded_image);
		DecodingImageWithGeometricTransform(tmp2, height, width, height, width, decoded_image_gt);
		DecodingImageWithScaling(tmp3, height, width, height, width, decoded_image_scaling);
		DecodingImageTotal(tmp4, height, width, height, width, decoded_image_total);

		CopyImage(decoded_image, height, width, tmp);
		CopyImage(decoded_image_gt, height, width, tmp2);
		CopyImage(decoded_image_scaling, height, width, tmp3);
		CopyImage(decoded_image_total, height, width, tmp4);

		ImageShow((char*)"Normal", decoded_image, height, width);
		ImageShow((char*)"Scaling", decoded_image_scaling, height, width);
		ImageShow((char*)"GT", decoded_image_gt, height, width);
		ImageShow((char*)"Total", decoded_image_total, height, width);

		printf("\n PSNR_normal_%d = %f", i, PSNR(image, decoded_image, height, width));
		printf("\n PSNR_scaling_%d = %f", i, PSNR(image, decoded_image_scaling, height, width));
		printf("\n PSNR_gt_%d = %f", i, PSNR(image, decoded_image_gt, height, width));
