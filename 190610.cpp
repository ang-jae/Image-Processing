...
Block** FindBiggestErrorBlocks()
{
	CopyBlocks(Blocks_Total, 32, Blocks_Small);
	int Err_MAX = 0;
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			if (Err_MAX < Blocks_Small[i / N][j / N].error)
			{
				Err_MAX = Blocks_Small[i / N][j / N].error;
				Blocks_Small[i/N][j/N].x = j;
				Blocks_Small[i/N][j/N].y = i;
				printf("\nErr_MAX = %d", Blocks_Small[i / N][j / N]);
				Blocks_Small[i / N][j / N].error = 0; //다음번 탐색을 위해 error를 0으로 바꿔준다.
			}
				printf("MAX ERR :%d Blocks_Small[%d][%d]", Err_MAX, i, j);
		}
	}
	return Blocks_Small;
}	
...
Block*DownDimension(Block**Blocks_Small)//버블정렬을 위해 1차원 배열로 만든다
{
	Block*Block_temp = BlockAlloc(height*width / (N*N));
	for (int i = 0; i < height / N; i++)
	{
		for (int j = 0; j < width / N; j++)
		{
			Block_temp[i*width/N + j] = Blocks_Small[i][j];
		}
	}
	return Block_temp;
}

void bubble_sort(Block Blocks, int n) {
	int temp;
