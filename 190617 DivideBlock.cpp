...
Block**Blocks_Big = BlockAlloc2(height / N, width / N);
...
void FindBiggestErrorBlock(int n)
{
	Blocks_Big = BlockAlloc2(height / N, width / N);
...
void DivideBlock(int**image_input, int N, int**im1, int**im2, int**im3, int**im4) //맨 왼쪽 위부터 오른쪽 아래 방향으로
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			if (i < N / 2 && j < N / 2)	im1[i / 2][j / 2] = image_input[i][j];
			else if (i > N / 2 && j < N / 2) im2[i / 2][j / 2] = image_input[i][j + N / 2];
			else if (i > N / 2 && j < N / 2) im3[i / 2][j / 2] = image_input[i + N / 2][j];
			else if (i > N / 2 && j > N / 2) im4[i / 2][j / 2] = image_input[i + N / 2][j + N / 2];
		}
	}
}
