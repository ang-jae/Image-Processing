...
int** image = ReadImage((char*)"LENA256.bmp", &height, &width);
int** input_image = ReadImage((char*)"LENA256.bmp", &height, &width);
...
int** decoded_image_gt = IntAlloc2(height, width);
int** decoded_image_scaling = IntAlloc2(height, width);
int** decoded_image_total = IntAlloc2(height, width);
...
int** Dblock2_mean_total = IntAlloc2(N, N);

Block Block_normal = SetBlock(0, 0, 0, 0, 0, 0.0);
Block Block_gt = SetBlock(0, 0, 0, 0, 0, 0.0);
Block Block_Scaled = SetBlock(0, 0, 0, 0, 0, 0.0);
Block Block_Total = SetBlock(0, 0, 0, 0, 0, 0.0);
Block Block_Small = SetBlock(0, 0, 0, 0, 0, 0.0);
...
Block**Blocks_Total = BlockAlloc2(height / N, width / N);
Block**Blocks_Small = BlockAlloc2(height / N, width / N);

int** block_cut1 = IntAlloc2(N / 2, N / 2);
int** block_cut2 = IntAlloc2(N / 2, N / 2);
int** block_cut3 = IntAlloc2(N / 2, N / 2); 
int** block_cut4 = IntAlloc2(N / 2, N / 2);

int**** IntAlloc4(int y, int x, int height, int width)
{
	int**** tmp;
	tmp = (int****)calloc(y, sizeof(int***));
	for (int i = 0; i < y; i++)
	{
		tmp[i] = (int***)calloc(x, sizeof(int**));
		for (int j = 0; j < x; j++)
		{
			tmp[i][j] = (int**)calloc(height, sizeof(int*));
			for (int k = 0; k < height; k++)
				tmp[i][j][k] = (int*)calloc(width, sizeof(int));
		}
...
/*
int**** Dblocks = IntAlloc4(height, width, 2 * N, 2 * N);
int**** Dblocks2 = IntAlloc4(height, width, N, N);
int**** Dblocks2_mean = IntAlloc4(height, width, N, N);
int**** Dblocks2_mean_gt = IntAlloc4(height, width, N, N);
int**** Dblocks2_mean_scaled = IntAlloc4(height, width, N, N);
int**** Dblocks2_mean_total = IntAlloc4(height, width, N, N);

void FindDblocks2_mean()
{
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			ReadBlock(image, height, width, i, j, 2 * N, 2 * N, Dblocks[i / (2 * N)][j / (2 * N)]);
			DownSize2(Dblocks[i / (2 * N)][j / (2 * N)], 2 * N, 2 * N, Dblocks2[i / (2 * N)][j / (2 * N)]);
			RemoveMean(Dblocks2[i / (2 * N)][j / (2 * N)], N, Dblocks2_mean[i / (2 * N)][j / (2 * N)]);
		}
	}
}
*/

Block BlockCompareWithDblocks(int**image_input, int y, int x)//x, y : block의 위치
{
	int Err_min = INT_MAX;
	//block
	ReadBlock(image_input, height, width, y, x, N, N, block);
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			ReadBlock(image, height, width, i, j, 2 * N, 2 * N, Dblock);
...
Block BlockCompareWithGeometricTransformedDblocks(int y, int x)
{
	int Err_min = INT_MAX;
	int geo_trans;
	//block
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			for (int k = 0; k < 8; k++)
			{
				...
				DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
				RemoveMean(Dblock2, N, Dblock2_mean);
				GeometricTransform(Dblock2_mean, Dblock2_mean_gt, N, N, k);
	...
	Block_gt.gt = geo_trans;
	Block_gt.error = Err_min;
	//printf("\nBlock_gt_(%dx, %dy) = , avg = %d, err = %d, gt = %d", Block_gt.x, Block_gt.y, Block_gt.avg, Block_gt.error, Block_gt.gt);
	return Block_gt;
}
...
Block BlockCompareWithScaledDblocks(int y, int x)
{
	int Err_min = INT_MAX;
	float alpha_out;
	int x_temp = 0, y_temp = 0; // if문 안에서 구조체로 전달이 잘 되지 않아서 추가
								//block
	ReadBlock(image, height, width, y, x, N, N, block);
	//ImageShow((char*)"test", block, N, N);
	Block_Scaled.avg = RemoveMean(block, N, block_mean);
	//Dblocks
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			for (float alpha = 0.3; alpha < 1.1; alpha += 0.1)
			{
				...
				DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
				RemoveMean(Dblock2, N, Dblock2_mean);
				Scaling(Dblock2_mean, alpha, N, N, Dblock2_mean_scaled);
				Block_Scaled.error = FindErr(block_mean, Dblock2_mean_scaled, 0, 0, N, N, N, N);
				//printf("\nerr = %d, alpha = %f", Block_Scaled.error, alpha);
				if (Block_Scaled.error < Err_min)
				{
					Err_min = Block_Scaled.error;
					alpha_out = alpha;
					x_temp = j;
					y_temp = i;
					//printf("\nif문 %f 동작, alpha = %f", alpha, Block_Scaled.alpha);
				}
			}
		}
	}
	Block_Scaled.alpha = alpha_out;
	Block_Scaled.x = x_temp;
	Block_Scaled.y = y_temp;
	Block_Scaled.error = Err_min;
	return Block_Scaled;
}
Block BlockCompareWithTotalDblocks(int y, int x)
{
	int Err_min = INT_MAX;
	int geo_trans_out, geo_trans;
	float alpha, alpha_out;
	int x_temp = 0, y_temp = 0; // if문 안에서 구조체로 전달이 잘 되지 않아서 추가
	ReadBlock(image, height, width, y, x, N, N, block);
	Block_Total.avg = RemoveMean(block, N, block_mean);
	//Dblocks
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			for (float alpha = 0.3; alpha < 1.1; alpha += 0.1)
			{
				for (int geo_trans = 0; geo_trans < 8; geo_trans++)
				{
					Scaling(Dblock2_mean, alpha, N, N, Dblock2_mean_scaled);
					GeometricTransform(Dblock2_mean_scaled, Dblock2_mean_total, N, N, geo_trans);
					Block_Total.error = FindErr(block_mean, Dblock2_mean_total, 0, 0, N, N, N, N);
					if (Block_Total.error < Err_min)
					{
						Err_min = Block_Total.error;
						alpha_out = alpha;
	...
	Block_Total.x = x_temp;
	Block_Total.y = y_temp;
	//printf("\n(x, y) = (%d, %d)", Block_Total.x, Block_Total.y);
	Block_Total.error = Err_min;
	Block_Total.alpha = alpha_out;
	Block_Total.gt = geo_trans_out;
	return Block_Total;
}
...
{
	Blocks_copied = BlockAlloc2(height / N, width / N);
	for (int i = 0; i < length; i++)
	{
		for (int j = 0; j < length; j++)
		{
			Blocks_copied[i][j] = SetBlock(Blocks[i][j].x, Blocks[i][j].y, Blocks[i][j].avg, Blocks[i][j].gt, Blocks[i][j].error, Blocks[i][j].alpha);
		}
	}
}
Block** FindBiggestErrorBlock()
{
	Block**Blocks_Big = BlockAlloc2(32, 32);
	CopyBlocks(Blocks_Total, 32, Blocks_Big);
	...
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			if (Err_MAX < Blocks_Big[i / N][j / N].error)
			{
				Err_MAX = Blocks_Big[i / N][j / N].error;
				Blocks_Big[i / N][j / N].x = j;
				Blocks_Big[i / N][j / N].y = i;
				printf("\nErr_MAX = %d", Blocks_Big[i / N][j / N].error);
				Blocks_Big[i / N][j / N].error = 0; //다음번 탐색을 위해 error를 0으로 바꿔준다.
				printf("\nErr_MAX :%d Blocks_Big[%d][%d]", Err_MAX, i / N, j / N);
			}
...
void DivideBlock(int**image_input, int y, int x, int N, int**im1, int**im2, int**im3, int**im4) //맨 왼쪽 위부터 오른쪽 아래 방향으로
{
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			if (i <= N / 2 && j <= N / 2) im1[i][j] = image_input[i][j];
			else if (i <= N / 2 && j >= N / 2) im2[i][j] = image_input[i][j];
			else if (i >= N / 2 && j <= N / 2) im3[i][j] = image_input[i][j];
			else if (i >= N / 2 && j >= N / 2) im4[i][j] = image_input[i][j];
		}
	}
}

Block BlockCompareWithSmallDblocks(int y, int x)
{
	int** im1 = IntAlloc2(4, 4);
	int** im2 = IntAlloc2(4, 4);
	int** im3 = IntAlloc2(4, 4);
	int** im4 = IntAlloc2(4, 4);

	for (int i = 0; i < 15; i++)
	{
		for (int j = 0; j < 15; j++)
		{
			Blocks_Small = FindBiggestErrorBlock();
			ReadBlock(image, height, width, Blocks_Small[i][j].y, Blocks_Small[i][j].x, N, N, block);
			DivideBlock(block, Blocks_Small[i][j].y, Blocks_Small[i][j].x, N, im1, im2, im3, im4);
		}
	}
	int Err_min = INT_MAX;
	int geo_trans_out, geo_trans;
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		...
		{
			ReadBlock(image, height, width, i, j, 2 * N, 2 * N, Dblock);
			DownSizeN(Dblock, 2 * N, 2 * N, 4, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
			for (float alpha = 0.3; alpha < 1.1; alpha += 0.1)
			{
				for (int geo_trans = 0; geo_trans < 8; geo_trans++)
				{
					int temp[4];
					Blocks_Small[i][j].avg = RemoveMean(Dblock, N, Dblock2_mean);
					Scaling(Dblock2_mean, alpha, N, N, Dblock2_mean_scaled);
					GeometricTransform(Dblock2_mean_scaled, Dblock2_mean_total, N, N, geo_trans);
					temp[0] = FindErr(im1, Dblock2_mean_total, 0, 0, N, N, N, N);
					temp[1] = FindErr(im2, Dblock2_mean_total, 0, 0, N, N, N, N);
					temp[2] = FindErr(im3, Dblock2_mean_total, 0, 0, N, N, N, N);
					temp[3] = FindErr(im4, Dblock2_mean_total, 0, 0, N, N, N, N);
					for (int m = 0; m < 3; m++)
					{
						if (temp[m] < temp[m + 1]) Blocks_Small[i][j].error = temp[i];
					}

					if (Blocks_Small[i][j].error < Err_min)
					{
						Err_min = Blocks_Small[i][j].error;
						alpha_out = alpha;
						geo_trans_out = geo_trans;
						x_temp = j;
						y_temp = i;
	...
	Block_Small.x = x_temp;
	Block_Small.y = y_temp;
	//printf("\n(x, y) = (%d, %d)", Block_Total.x, Block_Total.y);
	Block_Small.error = Err_min;
	Block_Small.alpha = alpha_out;
	Block_Small.gt = geo_trans_out;
	return Block_Small;
}
void EncodingImage()
{
	for (int i = 0; i < height / N; i++)
	{
		...
		{
			Blocks[i][j] = BlockCompareWithDblocks(image, i*N, j*N);
			//printf("\nBlocks[%d][%d]: (x, y) = (%d, %d), avg = %d", i, j, Blocks[i][j].x, Blocks[i][j].y, Blocks[i][j].avg);
		}
	}
}

void EncodingImageWithGeometricTransform()
{
	for (int i = 0; i < height / N; i++)
	{
		for (int j = 0; j < width / N; j++)
		{
			Blocks_GT[i][j] = BlockCompareWithGeometricTransformedDblocks(i*N, j*N);
			//printf("\nBlocks_gt[%d][%d]: (x, y) = (%d, %d), avg = %d, gt = %d", i, j, Blocks_GT[i][j].x, Blocks_GT[i][j].y, Blocks_GT[i][j].avg, Blocks_GT[i][j].gt);
		}
	}
}

void EncodingImageWithScaling()
{
	for (int i = 0; i < height / N; i++)
	{
		for (int j = 0; j < width / N; j++)
		{
			Blocks_Scaled[i][j] = BlockCompareWithScaledDblocks(i*N, j*N);
			//printf("\nBlocks_Scaled[%d][%d]: (x, y) = (%d, %d), avg = %d, alpha = %f", i, j, Blocks_Scaled[i][j].x, Blocks_Scaled[i][j].y, Blocks_Scaled[i][j].avg, Blocks_Scaled[i][j].alpha);
		}
	}
}

void EncodingImageTotal()
{
	for (int i = 0; i < height / N; i++)
	{
		for (int j = 0; j < width / N; j++)
		{
			Blocks_Total[i][j] = BlockCompareWithTotalDblocks(i*N, j*N);
	
			//printf("\nBlocks_Total[%d][%d]: (x, y) = (%d, %d), avg = %d, alpha = %f, gt = %d", i, j, Blocks_Total[i][j].x, Blocks_Total[i][j].y, Blocks_Total[i][j].avg, Blocks_Total[i][j].alpha, Blocks_Total[i][j].gt);
		}
	}
}

void EncodingImageSmall()
{
	for (int i = 0; i < height / N; i++)
	{
		for (int j = 0; j < width / N; j++)
		{
			Blocks_Small[i][j] = BlockCompareWithSmallDblocks(i * 4, j * 4);
...
void DecodingImage(int**image_input, int height_in, int width_in, int height_out, int width_out, int**decoded_image)
{
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
		{
			ReadBlock(image_input, height_in, width_in, Blocks[i / N][j / N].y, Blocks[i / N][j / N].x, 2 * N, 2 * N, Dblock);
			DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
			for (int k = 0; k < N; k++)
			{
...
void DecodingImageWithGeometricTransform(int**image_input, int width_in, int height_in, int width_out, int height_out, int**decoded_image_gt)
{
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
		{
			ReadBlock(image_input, height_in, width_in, Blocks_GT[i / N][j / N].y, Blocks_GT[i / N][j / N].x, 2 * N, 2 * N, Dblock);
			DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
			GeometricTransform(Dblock2_mean, Dblock2_mean_gt, N, N, Blocks_GT[i / N][j / N].gt);
			for (int k = 0; k < N; k++)
			{
				for (int l = 0; l < N; l++)
				{
					...
					{
						decoded_image_gt[i + k][j + l] = Dblock2_mean_gt[k][l] + Blocks_GT[i / N][j / N].avg;
						if (decoded_image_gt[i + k][j + l] > 255) decoded_image_gt[i + k][j + l] = 255;
						else if (decoded_image_gt[i + k][j + l] < 0) decoded_image_gt[i + k][j + l] = 0;
					}
				}
			}
		}
...
void DecodingImageWithScaling(int**image_input, int height_in, int width_in, int height_out, int width_out, int**decoded_image_scaling)
{
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
		{
			ReadBlock(image_input, height_in, width_in, Blocks_Scaled[i / N][j / N].y, Blocks_Scaled[i / N][j / N].x, 2 * N, 2 * N, Dblock);
			DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
			Scaling(Dblock2_mean, Blocks_Scaled[i / N][j / N].alpha, N, N, Dblock2_mean_scaled);
			for (int k = 0; k < N; k++)
			{
				for (int l = 0; l < N; l++)
				{
					...
					{
						decoded_image_scaling[i + k][j + l] = Dblock2_mean_scaled[k][l] + Blocks_Scaled[i / N][j / N].avg;
						if (decoded_image_scaling[i + k][j + l] > 255) decoded_image_scaling[i + k][j + l] = 255;
						else if (decoded_image_scaling[i + k][j + l] < 0) decoded_image_scaling[i + k][j + l] = 0;
					}
				}
			}
		}
...
void DecodingImageTotal(int**image_input, int height_in, int width_in, int height_out, int width_out, int**decoded_image_total)
{
	for (int i = 0; i < height_out; i += N)
	{
		for (int j = 0; j < width_out; j += N)
		{
			ReadBlock(image_input, height_in, width_in, Blocks_Total[i / N][j / N].y, Blocks_Total[i / N][j / N].x, 2 * N, 2 * N, Dblock);
			DownSize2(Dblock, 2 * N, 2 * N, Dblock2);
			RemoveMean(Dblock2, N, Dblock2_mean);
			Scaling(Dblock2_mean, Blocks_Total[i / N][j / N].alpha, N, N, Dblock2_mean_scaled);
			GeometricTransform(Dblock2_mean_scaled, Dblock2_mean_total, N, N, Blocks_Total[i / N][j / N].gt);
			for (int k = 0; k < N; k++)
			{
				for (int l = 0; l < N; l++)
				{
					...
					{
						decoded_image_total[i + k][j + l] = Dblock2_mean_total[k][l] + Blocks_Total[i / N][j / N].avg;
						if (decoded_image_total[i + k][j + l] > 255) decoded_image_total[i + k][j + l] = 255;
						else if (decoded_image_total[i + k][j + l] < 0) decoded_image_total[i + k][j + l] = 0;
					}
				}
			}
		}
...
Block*BlockAlloc(int length)
{
	Block* tmp;
	tmp = (Block*)calloc(length, sizeof(Block));
	return(tmp);
}
...
void main()
{
	int**im1 = IntAlloc2(height/2, width/2);
	int**im2 = IntAlloc2(height / 2, width / 2);
	int**im3 = IntAlloc2(height / 2, width / 2);
	int**im4 = IntAlloc2(height / 2, width / 2);
	DivideBlock(image, 0, 0, 32, im1, im2, im3, im4);
	ImageShow((char*)"original", image, height / 2, width / 2);
	ImageShow((char*)"divided", im1, height/2, width/2);
	ImageShow((char*)"divided", im2, height / 2, width / 2);
	ImageShow((char*)"divided", im3, height / 2, width / 2);
	ImageShow((char*)"divided", im4, height / 2, width / 2);
	//FindDblocks2_mean();
	EncodingImageTotal();
	Block**BiggestBlocks = FindBiggestErrorBlock();
	EncodingImageSmall();

	int** tmp = (int**)IntAlloc2(height, width);

	for (int i = 0; i < 100; i++)
	{
		DecodingImageTotal(tmp, height, width, height, width, decoded_image_total);
		CopyImage(decoded_image_total, height, width, tmp);
		ImageShow((char*)"Total", decoded_image_total, height, width);
		printf("\n PSNR_total_%d = %f", i, PSNR(image, decoded_image_total, height, width));
	}

}
//Error값이 너무 큰 경우, block을 4분의1로 쪼개서 같은 작업을 한다.
//구해야 되는 avg : only block
//190529
//배열과 포인터 차이점
//int A[2][3];
//int **B = IntAlloc2(2,3);
...
//		B[i][j] = i + j;
//	}
//하지만 메모리 할당 과정에서 A는 순서대로 6개를 잡고,
//B는 B[0], B[1] 포인터 먼저 잡고 각각에대해서 또 잡는다.
//따라서, 포인터는 배열보다 느리다. 그래서 보통 image processing할 때는 1차원 배열을 쓴다.
//발표할때 2가지 버전으로 발표.

/////발표//////
//1. LENA256.bmp, N=8로 float a = PSNR계산(image, decoded_image, height, width) -> e-campus 숫자가 높을수록 좋다
//2. N=8, N=4(전체의 20% -> err가 큰  순서대로 20% 선택해서 4등분, avg, x, y, alpha, gt 구한다)
//   float b = PSNR
//학기말 프로젝트 -> 속도, PSNR(2에 대해)
