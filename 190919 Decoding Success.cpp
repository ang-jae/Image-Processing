...
Block**EncodingImageWithGeometricTransform(int**image, int height, int width, int**block, int**block_mean,
	int** Dblock, int**Dblock2, int**Dblock2_mean, int**Dblock2_mean_gt, int N)
{
	Block**Blocks = BlockAlloc2(height / N, width / N);
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			...
			Blocks[i/N][j/N] = BlockCompareWithGeometricTransformedDblocks(image, height, width, block, block_mean, Dblock, Dblock2,
				Dblock2_mean, Dblock2_mean_gt, i, j, N, Blocks[i/N][j/N].gt);
...
void main()
{
	...
	*/
	...
	Blocks = EncodingImageWithGeometricTransform(image, height, width, block, block_mean,	Dblock, Dblock2, Dblock2_mean, Dblock2_mean_gt, N);
