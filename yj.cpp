...
int** image = ReadImage((char*)"LENA128.bmp", &height, &width);
int** input_image = ReadImage((char*)"LENA128.bmp", &height, &width);
...
Block BlockCompareWithDblocks(int**image_input, int y, int x)//x, y : block의 위치
{
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			Block_normal.error = FindErrRMS(block_mean, Dblock2_mean, 0, 0, N, N, N, N);
...
Block BlockCompareWithScaledDblocks(int y, int x, float alpha)
{
	...
	for (int i = 0; i < height - 2 * N; i++)
	{
		for (int j = 0; j < width - 2 * N; j++)
		{
			...
			Block_Scaled.error = FindErrRMS(block_mean, Dblock2_mean_scaled, 0, 0, N, N, N, N);
...
void EncodingImageTotal()
{
	for (int i = 0; i < height; i += N)
	{
		for (int j = 0; j < width; j += N)
		{
			...
			printf("\nBlocks_Total[%d][%d]: (x, y) = (%d, %d), avg = %d, alpha = %f, gt = %d", i, j, Blocks_Total[i / N][j / N].x, Blocks_Total[i / N][j / N].y, Blocks_Total[i / N][j / N].avg, Blocks_Total[i / N][j / N].alpha, Blocks_Total[i / N][j / N].gt);
		}
	}
}
